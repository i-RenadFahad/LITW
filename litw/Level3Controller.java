...
public class Level3Controller implements Initializable {
   ...
   static boolean pass =false;
    ...
    public void initialize(URL url, ResourceBundle rb) {
       AudioClip note =new AudioClip(this.getClass().getResource("/sound/level2.mp3").toString());
        note.play(); 
...
IB.play();
                    ...
                    if (sec <= 0 && pass == false)
                       {
                        try {
                            time.stop();
                            Parent rt = FXMLLoader.load(getClass().getResource("fail.fxml"));
  ...
  private void ButtonBack(ActionEvent event) throws IOException {
      ...
      Scene mapscen = new Scene(maproot);
